...
Adafruit_NeoPixel strip = Adafruit_NeoPixel((PIXEL_PER_SEGMENT * 7 * PIXEL_DIGITS), PIXEL_PIN, NEO_GRB + NEO_KHZ800);
...
void writeDigit(int index, int val) {
  ...
  for (int i = 6; i >= 0; i--) {
    ...
    if (digit & 0x01 != 0) {
      ...
      if (index == 4 || index == 5 ) color = strip.Color(Brightness, 0,  Brightness);
    }
    else
